...
def get_indices(pre_unit_types,n_pre_neurons,post_unit_types,n_post_neurons):
    pre_type_indices = torch.tile(torch.tensor(pre_unit_types[None, :, None]),
                                   (1,1,n_post_neurons))


    post_type_indices = torch.tile(torch.tensor(post_unit_types[None, :, None]),
                                (1,1, n_pre_neurons))

    indices = torch.stack((pre_type_indices, post_type_indices.permute(0, 2, 1)),
                       axis=-1)
...
def to1d_CNN(a,x,y,z,out_channel,in_channel,num_out,num_inp):
    return (a * num_inp * in_channel * num_out) + (x * num_inp *num_out) + (y * num_inp) + z
...
def get_indices_CNN(n_neurons_arr,unit_types_arr):
    ...
    pre_type_indices = torch.tile(torch.tensor(pre_unit_types[None, None, None, :]),
                                      (1,n_post_channels,n_pre_channels , n_post_neurons, 1))


    post_type_indices = torch.tile(torch.tensor(post_unit_types[None, None, :, None]),
                                      (1,n_post_channels,n_pre_channels , 1, n_pre_neurons))



    pre_channel_indices = torch.tile(torch.tensor(pre_channel_types[None, :, None, None]),
                                      (1,n_post_channels, 1 , n_post_neurons, n_pre_neurons))



    post_channel_indices = torch.tile(torch.tensor(post_channel_types[:, None, None, None]),
                                      (1,1, n_pre_channels, n_post_neurons, n_pre_neurons))
    

    indices = torch.stack((post_channel_indices, pre_channel_indices,
                           post_type_indices, pre_type_indices),
                          axis=-1)
