...
model_cf_arr = ['mean','sampled_0','sampled_1','sampled_2','sampled_3','sampled_4','sampled_5','sampled_6','sampled_7','sampled_8','sampled_9']
# ratio_arr = [1]
...
def trainMNIST(model, model_name, device, train_loader, optimizer, epoch, epochs, test_loader=None):
    ...
    for nepoch in range(epochs):
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            correct_per_batch.append(pred.eq(target.view_as(pred)).sum().item()/pred.size()[0])
...
if __name__ == '__main__': 
    ...
    home_path = './WS-BNN/results_Big_MLP_mean_exp/2023-07-24'

    
    exp_folder = [name for name in os.listdir(home_path) if os.path.isdir(os.path.join(home_path, name))]
    ...
    MNIST_validation_dataset,remainder = torch.utils.data.random_split(MNIST_train_dataset, [test_set_length, remainder_length])
    
    train_loader_mnist = torch.utils.data.DataLoader(MNIST_train_dataset, batch_size=128, shuffle=True, **kwargs)
    test_loader_mnist = torch.utils.data.DataLoader(MNIST_test_dataset, batch_size=128, shuffle=True, **kwargs)
    valid_loader_mnist = torch.utils.data.DataLoader(MNIST_validation_dataset, batch_size=128, shuffle=True, **kwargs)
    


    for step_idx in range(number_of_epochs):
        CorrectPreTrain[step_idx], correct_per_batch_train, correct_per_batch_test = trainMNIST(model, 'MLP', device, train_loader_mnist, optimizer, step_idx, 1, test_loader=test_loader_mnist)
