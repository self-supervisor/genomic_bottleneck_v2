...
class TrainableRandomDistribution_weight_share(nn.Module):
    ...
    def __init__(self, mu, rho,indices):
        ...
        self.indices.flat = gather_nd_torch_dims_flat(self.weight_mu_share, self.indices, batch_dim=1)
...
class BayesianGRU(BayesianRNN):
    ...
    def __init__(self,
        ...
        if self.WS == False:
            ...
            self.pnet_size =  in_features*out_features*4 + out_features*out_features*4
            self.gnet_size =  neuron_types_in*neuron_types_out*4 + neuron_types_out*neuron_types_out*4
            
            n_in_neurons_per_type = find_num_neuron_per_type(in_features,self.neuron_types_in)
            in_neuron_type_ids = np.concatenate([i * np.ones(n) for i, n in enumerate(n_in_neurons_per_type)],
                                                     axis=0).astype(np.int32)

            n_out_neurons_per_type = find_num_neuron_per_type(out_features*4,self.neuron_types_out*4)
            out_neuron_type_ids = np.concatenate([i * np.ones(n) for i, n in enumerate(n_out_neurons_per_type)],
                                                     axis=0).astype(np.int32)

            indices = get_indices(in_neuron_type_ids,in_features,out_neuron_type_ids,out_features*4)
            # Variational weight parameters and sample for weight ih
            self.weight_ih_mu = nn.Parameter(torch.Tensor(1,self.neuron_types_out * 4,self.neuron_types_in).normal_(posterior_mu_init, 0.1))
            self.weight_ih_rho = nn.Parameter(torch.Tensor(1,self.neuron_types_out * 4,self.neuron_types_in).normal_(posterior_rho_init, 0.1))
            self.weight_ih_sampler = TrainableRandomDistribution_weight_share(self.weight_ih_mu, self.weight_ih_rho,indices)
            self.weight_ih = None
            
            
            n_in_neurons_per_type = find_num_neuron_per_type(out_features,self.neuron_types_out)
            in_neuron_type_ids = np.concatenate([i * np.ones(n) for i, n in enumerate(n_in_neurons_per_type)],
                                                     axis=0).astype(np.int32)
            ...
            indices = get_indices(in_neuron_type_ids,out_features,out_neuron_type_ids,out_features*4)

            # Variational weight parameters and sample for weight hh
            self.weight_hh_mu = nn.Parameter(torch.Tensor(1,self.neuron_types_out*4, self.neuron_types_out).normal_(posterior_mu_init, 0.1))
            self.weight_hh_rho = nn.Parameter(torch.Tensor(1,self.neuron_types_out*4, self.neuron_types_out).normal_(posterior_rho_init, 0.1))
            self.weight_hh_sampler = TrainableRandomDistribution_weight_share(self.weight_hh_mu, self.weight_hh_rho,indices)
            self.weight_hh = None
...
class BayesianLinear(BayesianModule):
    ...
    def __init__(self,
        ...
        if self.WS == False:
            ...
            self.neuron_types_in = neuron_types_in
            self.neuron_types_out = neuron_types_out
            n_in_neurons_per_type = find_num_neuron_per_type(in_features,neuron_types_in)
            in_neuron_type_ids = np.concatenate([i * np.ones(n) for i, n in enumerate(n_in_neurons_per_type)],
                                                     axis=0).astype(np.int32)

            n_out_neurons_per_type = find_num_neuron_per_type(out_features,neuron_types_out)
            out_neuron_type_ids = np.concatenate([i * np.ones(n) for i, n in enumerate(n_out_neurons_per_type)],
                                                     axis=0).astype(np.int32)


            indices = get_indices(in_neuron_type_ids,in_features,out_neuron_type_ids,out_features)
            self.weight_mu_share = nn.Parameter(torch.Tensor(1,neuron_types_in, neuron_types_out).normal_(posterior_mu_init, 0.1))
            self.weight_rho_share = nn.Parameter(torch.Tensor(1,neuron_types_in, neuron_types_out).normal_(posterior_rho_init, 0.1))
           
            
            self.weight_sampler = TrainableRandomDistribution_weight_share(self.weight_mu_share, self.weight_rho_share,indices)
...
class BayesianConv2d(BayesianModule):
    ...
    def __init__(self,
        ...
        if not self.WS:
            ...
            self.pnet_size = out_channels*(in_channels // groups)*kernel_size[0]*kernel_size[1]
            self.gnet_size = n_out_channels_types*(n_in_channels_types // groups)*n_out_kernel_types*n_in_kernel_types
            
            n_kernel_in_neurons_per_type = find_num_neuron_per_type(kernel_size[0],n_in_kernel_types)
            pre_unit_types = np.concatenate([i * np.ones(n) for i, n in enumerate(n_kernel_in_neurons_per_type)],
                                                                 axis=0).astype(np.int32)

            n_kernel_out_neurons_per_type = find_num_neuron_per_type(kernel_size[1],n_out_kernel_types)
            post_unit_types = np.concatenate([i * np.ones(n) for i, n in enumerate(n_kernel_out_neurons_per_type)],
                                                                 axis=0).astype(np.int32)


            n_in_channel_neurons_per_type = find_num_neuron_per_type(self.in_channels,n_in_channels_types)
            pre_channel_types = np.concatenate([i * np.ones(n) for i, n in enumerate(n_in_channel_neurons_per_type)],
                                                                 axis=0).astype(np.int32)


            n_kernel_out_neurons_per_type = find_num_neuron_per_type(self.out_channels,n_out_channels_types)
            post_channel_types = np.concatenate([i * np.ones(n) for i, n in enumerate(n_kernel_out_neurons_per_type)],
                                                                 axis=0).astype(np.int32)
           
            indices = get_indices_CNN([self.out_channels,self.in_channels,kernel_size[1],kernel_size[0]],[post_channel_types,pre_channel_types,post_unit_types,pre_unit_types])
            self.indices = indices
            self.weight_mu = nn.Parameter(torch.Tensor(1,n_out_channels_types, n_in_channels_types // groups,n_out_kernel_types,n_in_kernel_types).normal_(posterior_mu_init, 0.1))
            self.weight_rho = nn.Parameter(torch.Tensor(1,n_out_channels_types, n_in_channels_types // groups, n_out_kernel_types,n_in_kernel_types).normal_(posterior_rho_init, 0.1))
            self.weight_sampler = TrainableRandomDistribution_weight_share_CNN(self.weight_mu, self.weight_rho,indices)
    ...
    def forward_frozen(self, x):
        ...
        w = torch.squeeze(gather2D(self.weight_sampler.weight_mu_share, self.weight_sampler.indices, 
                                             self.weight_sampler.indices.flat))
    ...
    def sample_layer_weights(self, expected_value=True):
        if self.WS and expected_value:
            w = gather2D(self.weight_sampler.weight_mu_share, self.weight_sampler.indices, 
                                             self.weight_sampler.indices.flat)
